...
void TrafficLight::waitForGreen() {
  ...
  // returns.
  while (true) {
    TrafficLightPhase phase = _messenger.receive();
...
void TrafficLight::cycleThroughPhases() {
  ...
  int cycleDuration = distr(eng);  // Duration between cycles in ms
  ...
  while (true) {
    ...
    if (timeSinceLastUpdate >= cycleDuration) {
      ...
      _messenger.send(std::move(message));
      // Reset stop whatch
      lastUpdate = std::chrono::system_clock::now();
    }
  }
